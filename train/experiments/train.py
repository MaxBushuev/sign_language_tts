...
from transformers import AutoProcessor, BitsAndBytesConfig, AutoModelForImageTextToText, TrainerCallback
...
def main(args):
    ...
    print(ds)
    ds = ds.filter(lambda x: op.isfile(op.join(args.video_root, x["SENTENCE_NAME"] + ".mp4")))
    print(ds)
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            ...
            # print(op.join(args.video_root, example["SENTENCE_NAME"] + ".mp4"))
            messages = [
    ...
    if not op.isdir(args.output_path):
    ...
    class MemoryAwareTrainerCallback(TrainerCallback):
    ...
    trainer = Trainer(
        ...
        callbacks=[MemoryAwareTrainerCallback()]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_id", default="models/models--HuggingFaceTB--SmolVLM2-500M-Video-Instruct")
